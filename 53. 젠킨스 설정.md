...
## jenkins 구성
+ 도커 설치
+ 컨테이너내 젠킨스 홈 폴더와 마운트 시킬 디렉토리 생성 및 권한 부여
...
+ 젠킨스 컨테이너 생성
    ...
    01. -v /docker/jenkins:/var/jenkins_home
        - 젠킨스 컨테이너의 설정을 호스트 서버와 공유함으로써, 컨테이너가 삭제되는 경우에도 설정을 유지할수 있게 해줌.
    02. -v /usr/bin/docker:/usr/bin/docker -v /var/run/docker.sock:/var/run/docker.sock
        - 젠킨스 컨테이너에서도 호스트 서버의 도커를 사용하기 위한 바인딩.
    03. -p 8080:8080
        - 젠킨스의 defaul port 인 8080 을 호스트 서버와 매핑.
    04. 현재 구성된 젠킨스의 경우 **jenkins/jenkins:lts** 이미지를 기반으로 구성.
...
+ **젠킨스 plugin 설치**
    ...
    5. jenkins 관리 > Plugins > Available Plugins > Gradle Plugin
        -  젠킨스 버전에 따라 디폴트로 설치 되어 있을 수 있음
  >![jenkins_plugin.png](images%2Fjenkins%2Fjenkins_plugin.png)
...
1. jenkins 관리 > Systems > Publish over SSH (plugin 설치) > 배포할 서버 private key 세팅
    - **name**값 파이프라인에서 사용
      > ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_ssh.png)

2. jenkins 관리 > Tools > Gradle installations > 7.6 버전 설정
    - **name**값 파이프라인에서 사용
      > ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_gradle.png)

3. jenkins 관리 > Tools > Maven installations > 3 버전 설정
    - **name**값 파이프라인에서 사용
      > ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_mvn.png)

4. jenkins 관리 > Tools > Docker installations (plugin 설치) > 설정
    - **name**값 파이프라인에서 사용
      > ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_docker.png)
...
5. jenkins 관리 > Managed files(plugin 설치) > Add a new config  > Maven settings.xml > 하기 코드 입력 및 생성
    ...
    - 현재 로컬 경로는 컨테이너 내 /wgms/web_maven에 위치
    - **ID**값 파이프라인에서 사용
      >   ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_file.png)

6. jenkins 관리 > Credentials >  add Credentials (프로젝트별 Git 계정 등록)
    - **ID**값 파이프라인에서 사용
      >   ![jenkins_ssh.png](images%2Fjenkins%2Fjenkins_credentials.png)
