...
#### Pipeline 변수 설정 
        ...
        def giturl = "https://github.com/golfzone/smartfield-api.git"  //git http
...
### wgms web
        ...
        def branch = "master"
        def gitCredentials = "gitlab-signin"
        def giturl = "http://172.19.156.10:8088/web-gms/wgms-web.git"
            ...
            stages {
                                ...
                                sshPublisherDesc(
                                    ...
                                    transfers: [
                                        sshTransfer(
                                            ...
                                            remoteDirectory: "/app", //배포할 위치
                                            execCommand: 
                                                "docker stop ${dockerImageName} || true && docker system prune -a -f && cd app && docker load -i ${dockerImageName}.tar && docker run -i -d -p 8030:8081 -e TZ=Asia/Seoul --name ${dockerImageName} ${dockerImageName}:0.0.1 && docker ps && rm -f ${dockerImageName}.tar"
                                            )
                                    ]
                                )
                            ]
                ...
                stage('Clean') {
                    steps {
                        sh "rm -rf ./*"
                        sh "rm -rf ./.git"
                        sh "rm -rf /var/jenkins_home/jobs/wgms_web_web01/*"
                    }
                    post {
                        failure {
                            echo 'Docker image build failure !'
